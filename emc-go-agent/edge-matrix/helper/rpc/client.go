...
import (
	...
	"errors"
	"fmt"
	"github.com/emc-go-agent/edge-matrix/chain"
	"github.com/emc-go-agent/edge-matrix/crypto"
	"github.com/emc-go-agent/edge-matrix/helper/hex"
...
func NewDefaultClient(chainId int64) *JsonRpcClient {
	return &JsonRpcClient{
		httpClient: NewDefaultHttpClient(),
		signer:     crypto.NewEIP155Signer(chain.AllForksEnabled.At(0), big.NewInt(chainId).Uint64()),
		rpcUrl:     DEFAULT_RPC_URL,
	}
}

func NewClientWithRpcUrl(chainId int64, rpcUrl string) *JsonRpcClient {
	return &JsonRpcClient{
		httpClient: NewDefaultHttpClient(),
		signer:     crypto.NewEIP155Signer(chain.AllForksEnabled.At(0), big.NewInt(chainId).Uint64()),
		rpcUrl:     rpcUrl,
